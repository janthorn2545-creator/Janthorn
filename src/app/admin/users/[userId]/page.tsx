...
export default async function UserDetailPage({ params }: { params: Promise<{ userId: string }> }) {
  ...
  if (!['superadmin', 'admin'].includes(myProfile?.role || '')) redirect('/employee/dashboard')
  ...
  const { data: enrollments } = await supabase
    .from('enrollments').select('*, courses(id, title, pass_score)')
    ...
    <div className="p-6 max-w-4xl">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 mb-4">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                u.role === 'superadmin' ? 'bg-yellow-100 text-yellow-800' :
                u.role === 'admin' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'
              }`}>
                {u.role === 'superadmin' ? '⭐ ผู้ดูแลหลัก' : u.role === 'admin' ? '🛡️ ผู้ดูแล' : '👤 พนักงาน'}
              </span>
            </div>
          ...
          <UserActions userId={userId} userName={u.full_name} currentRole={u.role} myRole={myProfile?.role || 'admin'} />
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          <div className="divide-y divide-gray-100">
              ...
              return (
                <div key={c.id} className="px-5 py-3">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${isExpired ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-700'}`}>
            ...
            {(!certs || certs.length === 0) && <p className="text-center py-6 text-sm text-gray-400">ยังไม่มีบัตร</p>}
          </div>
        </div>

        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="px-5 py-3 border-b border-gray-100">
            <span className="text-sm font-semibold text-gray-800">📚 คอร์สที่ลงทะเบียน</span>
          </div>
          <div className="divide-y divide-gray-100">
            {(enrollments || []).map((e: any) => (
              <div key={e.id} className="px-5 py-3">
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <div className="text-sm font-medium text-gray-800">{(e.courses as any)?.title}</div>
                ...
                {e.status === 'approved' && e.courses?.id && (
                  <div className="flex gap-1.5 flex-wrap">
                    {[
                      { action: 'reset_progress', label: 'รีเซ็ตวิดีโอ', color: 'amber' },
                      { action: 'reset_quiz', label: 'รีเซ็ตข้อสอบ', color: 'orange' },
                      { action: 'reset_all_course', label: 'รีเซ็ตทั้งหมด', color: 'red' },
                    ].map(btn => (
                      <Link key={btn.action}
                        href={`/admin/users/${userId}/reset?course=${e.courses.id}&action=${btn.action}`}
                        className={`text-xs border px-2 py-1 rounded-lg transition-colors ${
                          btn.color === 'red' ? 'border-red-200 text-red-600 hover:bg-red-50' :
                          btn.color === 'orange' ? 'border-orange-200 text-orange-600 hover:bg-orange-50' :
            ...
            {(!enrollments || enrollments.length === 0) && <p className="text-center py-6 text-sm text-gray-400">ยังไม่มี</p>}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          <div className="divide-y divide-gray-100">
            {(results || []).slice(0, 10).map((r: any) => {
              ...
              return (
                <div key={r.id} className="px-5 py-3 flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-sm font-bold flex-shrink-0 ${r.passed ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'}`}>
                    {pct}%
                  </div>
            ...
            {(!results || results.length === 0) && <p className="text-center py-6 text-sm text-gray-400">ยังไม่มีประวัติ</p>}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          <div className="divide-y divide-gray-100">
            ...
            {(!lessonProgress || lessonProgress.length === 0) && <p className="text-center py-6 text-sm text-gray-400">ยังไม่มีประวัติ</p>}
          </div>
        </div>
      </div>
